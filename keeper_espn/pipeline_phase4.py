...
def validate_phase4(advisor_packet: dict) -> list[str]:
    errors: list[str] = []
    required_sections = (
        "run_state",
        "team_state",
        "matchup",
        "taylor_made_roster",
        "actionable_player_pool",
        "material_intelligence",
        "material_deltas",
        "decision_queue",
        "weekly_result",
        "league_recap_summary",
        "automation_boundary",
    )
    for section in required_sections:
        if section not in advisor_packet:
            errors.append(f"Advisor packet missing section: {section}")
    if advisor_packet.get("automation_boundary", {}).get("strategy_decisions_embedded") is not False:
        errors.append("Advisor packet must not embed strategy decisions")
    if advisor_packet.get("automation_boundary", {}).get("final_tci_assigned") is not False:
        errors.append("Advisor packet must not assign final TCI")
    return errors
