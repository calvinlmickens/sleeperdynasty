...
from .phase5 import (
    apply_results_to_advisor_packet,
    build_league_results,
)
...
def validate_phase5(league_results: dict) -> list[str]:
    errors: list[str] = []
    for key in (
        "season",
        "week",
        ...
        if key not in league_results:
            errors.append(f"League results missing key: {key}")

    if league_results.get("results_status") == "FINAL_RESULTS":
        expected_teams = int(league_results.get("team_count") or 0)
        expected_matchups = expected_teams // 2 if expected_teams else 0
        if expected_matchups and int(league_results.get("matchup_count") or 0) != expected_matchups:
            errors.append("Final league results do not contain the expected number of matchups")
        if league_results.get("taylor_made_result") is None:
            errors.append("Final league results are missing Taylor Made")
        if not league_results.get("completeness", {}).get("player_actuals_complete"):
            errors.append("Final league results are missing complete player actuals")
    return errors
