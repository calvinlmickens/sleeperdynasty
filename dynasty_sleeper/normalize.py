...
def _roster_membership_assignment(
    roster: dict[str, Any],
    matchup: dict[str, Any],
) -> tuple[list[str], str]:
    """Return player membership for the requested week.

    Sleeper's roster endpoint is live/current and may contain post-week add/drops by
    the time a historical week is finalized. The weekly matchup endpoint preserves
    the player list that belonged to that weekly matchup, so use it whenever
    available. This keeps historical starter/bench analysis tied to the same weekly
    roster that Sleeper scored.
    """
    if "players" in matchup and matchup.get("players") is not None:
        return [str(x) for x in (matchup.get("players") or []) if x is not None], "WEEKLY_MATCHUP_PLAYERS"
    return [str(x) for x in (roster.get("players") or []) if x is not None], "ROSTER_CURRENT_PLAYERS_FALLBACK"
...
def normalize_league_state(
    ...
    """Create one normalized player-per-roster weekly snapshot.

    Stable keys are league_id, roster_id, sleeper_player_id. When weekly matchup
    data is supplied, both roster membership and starter/bench state come from
    Sleeper's matchup payload for the requested week. The live roster endpoint is
    used only as a fallback when weekly matchup data is unavailable.
    """
    ...
    for roster in rosters:
        ...
        roster_players, roster_source = _roster_membership_assignment(roster, matchup)
        starter_order_by_id = {
            player_id: idx
        ...
        for player_id in roster_players:
            ...
            rows.append({
                ...
                "roster_assignment_source": roster_source,
